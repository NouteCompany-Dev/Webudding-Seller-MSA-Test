...
export const rsaEncrypt = (str: string) => {
    if (str === "" || str == null) {
        ...
        let crpto = crypto.publicEncrypt({key: key, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING},Buffer.from(str))
...
export const rsaDencrypt = (str: string) => {
    if (str === "" || str == null) {
        ...
        let crpto = crypto.privateDecrypt({key: key, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING}, Buffer.from(str, 'base64'))
