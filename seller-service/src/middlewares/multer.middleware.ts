...
const fileStorage = multerS3({
    ...
    key: (_, file, cb) => {
        ...
        } else if (file.fieldname == 'backProfile') {
            location = `public/seller/info/${filename}`
        } else if (file.fieldname == 'certificate' || file.fieldname == 'family' || file.fieldname == 'legalRepresent' || file.fieldname == 'legalRepresentCertificate') {
...
export const fileUpload = { storage: fileStorage, limits: { fieldSize: 2 * 1024 * 1024 * 1024, fileSize: 2 * 1024 * 1024 * 1024 } }

export const sellerUpload = { storage: sellerStorage }

export const s3Delete = (param) => new AWS.S3(awsConfig).deleteObject(param, (err, data) => {
    if (err) {
